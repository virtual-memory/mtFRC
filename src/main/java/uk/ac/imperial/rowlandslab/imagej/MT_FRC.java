...
public class MT_FRC implements PlugIn
{
	...
	private void displayRunDialog()
	{
		...
		Integer[] tileSizeOptions = new Integer[]{64, 128, 256, 512, 1024};
		...
		comboBoxTileOptions.setSelectedItem(128);
		c.gridx = 1;
		panel.add(comboBoxTileOptions, c);

		JLabel labeltileSizeInfo = new JLabel("Size is in pixels relative to original image.");
		...
		okButton.addActionListener(new ActionListener()
		{
			...
			{
				okButtonClicked(radioButtonRois, radioButtonWholeImage, spinnerPixelSize, spinnerStepSize, comboBoxTileOptions, checkBoxShowPlot, checkBoxGenerateColormaps, spinnerColormapMin, spinnerColormapMax);
			}
		});
	...
	private void okButtonClicked(JRadioButton radioButtonRois, JRadioButton radioButtonWholeImage, JSpinner spinnerPixelSize, JSpinner spinnerStepSize, JComboBox<Integer> comboBoxTileOptions, JCheckBox checkBoxShowPlot, JCheckBox checkGenerateColormaps, JSpinner spinnerColormapMin, JSpinner spinnerColormapMax)
	{
		boolean useRois = false;
		if( radioButtonRois.isSelected() )
		{
		...
		// Set tile dimensions
		// Note that 'tileWidth' and 'tileHeight' use dimensions in pixels for sub-images.
		// Relative to the original image, the tile size is 2x these dimensions.
		int tileWidth = (int)comboBoxTileOptions.getSelectedItem() / 2;
		int tileHeight = tileWidth;

		BatchProcess.batchMeanTiledFRC(useRois, tileWidth, tileHeight, pixelSize, stepSize, isShowPlotEnabled, isGenerateColormapsEnabled, colormapUmMin, colormapUmMax);
